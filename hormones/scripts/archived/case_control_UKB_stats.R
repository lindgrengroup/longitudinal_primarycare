...
write.table(dat, "/well/lindgren/UKBIOBANK/samvida/hormone_ehr/LH_FSH/summarised_LH_FSH_case_control_UKB_pheno.txt", 
...
# Plot data demographics ----

cases <- subset(dat, dat$case)[, 1:18]
cases$group <- "case"
control_s1 <- subset(dat, dat$control_s1)[, 1:18]
control_s1$group <- "random full cohort"
control_s2 <- subset(dat, dat$control_s2)[, 1:18]
control_s2$group <- "random non-case cohort"
control_s3 <- subset(dat, dat$control_s3)[, 1:18]
control_s3$group <- "matched"

pdat <- bind_rows(cases, control_s1, control_s2, control_s3)

pdf("summarised_LH_FSH_case_control_UKB_pheno.pdf", onefile = T)

# Age
ggplot(pdat, aes(x = age_2020, color = group, fill = group)) +
  facet_wrap(~sex, scales = "free", nrow = 2) +
  geom_density(alpha = 0.2) +
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  labs(x = "Age in 2020", "Density")

# BMI
ggplot(pdat, aes(x = BMI, color = group, fill = group)) +
  facet_wrap(~sex, scales = "free", nrow = 2) +
  geom_density(alpha = 0.2) +
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  labs(x = "BMI (UKB)", "Density")

# number of live births
fdat <- subset(pdat, pdat$sex == "F")
fdat <- fdat %>% mutate(n_live_births = floor(n_live_births)) %>% 
  group_by(group, n_live_births) %>% 
  summarise(count = n()) %>% 
  mutate(freq = count/sum(count))

ggplot(fdat, aes(x = n_live_births, y = freq, color = group, fill = group)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  scale_x_continuous(limits = c(-0.5, 5.5), breaks = c(0:5), 
                     labels = c(0:5)) +
  labs(x = "Number of live births", y = "Frequency in cohort") 

# proportion to report: SSMT, oral contraceptives, HRT
fdat <- subset(pdat, pdat$sex == "F")
fdat <- fdat[, c("eid", "group", "SSMT", "ever_oral_contraceptive",
                 "ever_HRT")] %>% group_by(group) %>%
  summarise_at(c("SSMT", "ever_oral_contraceptive",
                 "ever_HRT"), mean, na.rm = T)
fdat <- pivot_longer(fdat, cols = -group,
                     names_to = "attribute", values_to = "percentage")

ggplot(fdat, aes(x = attribute, y = percentage, color = group, fill = group)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Dark2") +
  scale_x_discrete(labels = c("ever had SSMT", "ever used HRT",
                              "ever used oral contrac.")) +
  labs(y = "Frequency in cohort")
