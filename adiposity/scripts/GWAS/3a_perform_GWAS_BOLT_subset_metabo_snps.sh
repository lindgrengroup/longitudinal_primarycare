...
#$ -pe shmem 12
#$ -N lmm_slopes_adj_baseline_GWAS
#$ -j y
...
--phenoFile=/well/lindgren/UKBIOBANK/samvida/adiposity/gp_only/GWAS/traits_for_gwas/${STRATA_NAME}/lmm_slopes_adj_baseline_${STRATA_NAME}.txt \
--phenoCol=adj_trait \
...
--lmm \
--numThreads=8 \
--statsFile=/well/lindgren/UKBIOBANK/samvida/adiposity/gp_only/GWAS/BOLT_results/${STRATA_NAME}_lmm_slopes_adj_baseline_assoc_cal.stats.gz \
...
--statsFileBgenSnps=/well/lindgren/UKBIOBANK/samvida/adiposity/gp_only/GWAS/BOLT_results/${STRATA_NAME}_lmm_slopes_adj_baseline_assoc_imp.stats.gz \
--verboseStats

cd /well/lindgren/UKBIOBANK/samvida/adiposity/gp_only/GWAS/BOLT_results
cat ./${STRATA_NAME}_lmm_slopes_adj_baseline_assoc_cal.stats.gz \
./${STRATA_NAME}_lmm_slopes_adj_baseline_assoc_imp.stats.gz \
> ./${STRATA_NAME}_lmm_slopes_adj_baseline_assoc.stats.gz

# Create log file for filtering results
LOG_FILE="../log_files/post_GWAS/${STRATA_NAME}_lmm_slopes_adj_baseline.txt"
rm $LOG_FILE
touch $LOG_FILE
...
gunzip ./${STRATA_NAME}_lmm_slopes_adj_baseline_assoc.stats.gz 

PREQC=$(wc -l < ./${STRATA_NAME}_lmm_slopes_adj_baseline_assoc.stats)
...
${FILTER_LOC}/passed_maf_QC.txt ./${STRATA_NAME}_lmm_slopes_adj_baseline_assoc.stats \
...
# Save results
cp ./${STRATA_NAME}_tmp_QC/tmp_passed_biallelic.txt ./${STRATA_NAME}_lmm_slopes_adj_baseline_filtered.txt
gzip ./${STRATA_NAME}_lmm_slopes_adj_baseline_filtered.txt
rm -r ./${STRATA_NAME}_tmp_QC
