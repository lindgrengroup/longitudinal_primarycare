...
# Cleaned phenotypes from full primary care data (script in ../full_primary_care)
...
jumps <- lapply(cleaned_adiposity, function (df) {
...
# Log number of obsercations and individuals removed by jumps ----

lapply(PHENOTYPES, function (p) {
  orig_obs <- dim(cleaned_adiposity[[p]])[1]
  orig_indivs <- length(unique(cleaned_adiposity[[p]]$eid))
  ...
  sink(qc_log, append = T)
  cat(paste0("** PHENOTYPE **", p, "\n",  
             "**FILTER** EXCLUDED, Unreasonable jump > 3 S.D. away from popn.: ", 
             "\n",
