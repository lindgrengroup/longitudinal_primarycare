...
qcd_slopes <- lapply(PHENOTYPES, function (p) {
  res <- lapply(SEX_STRATA, function (sx) {
    ...
    qc_log_file <- paste0("/well/lindgren-ukbb/projects/ukbb-11867/samvida/adiposity/gp_only/GWAS/log_files/",
                          p, "_", sx, "_UNRELATED_qc.txt")
...
ids_for_gwas <- lapply(PHENOTYPES, function (p) {
  res <- lapply(SEX_STRATA, function (sx) {
    ...
    write.table(to_write, 
                paste0("/well/lindgren-ukbb/projects/ukbb-11867/samvida/adiposity/gp_only/GWAS/sample_qc/", 
                       p, "_", sx, "_UNRELATED_ids_passed_qc.txt"), 
