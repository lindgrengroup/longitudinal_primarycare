...
filepath_main <- "/well/lindgren-ukbb/projects/ukbb-11867/samvida/adiposity/2204_models/GWAS/post_GWAS/BMI_sex_comb/classify_lmm_intercept_variants/"
...
sumstats <- read.table("tmp_sumstats_gcta.txt",
...
isUncorr <- function (maybe_novel) {
  ld_file <- read.table(paste0(filepath_main, "tmp_ukbb_ld/", maybe_novel,
                               "_region.ld"),
                        header = F, stringsAsFactors = F)
  colnames(ld_file) <- c("CHR_A", "POS_A", "SNP_A",
                         "CHR_B", "POS_B", "SNP_B", "r2")
  return (all(ld_file$r2 < 0.1))
}
...
# Function to return nearest or most highly correlated buddy to a SNP ----

# Get buddy based on either R2 or distance
getBuddy <- function (lead_snp, metric = "r2") {
  ld_file <- read.table(paste0(filepath_main, "tmp_ukbb_ld/", lead_snp,
                               "_region.ld"),
                        header = F, stringsAsFactors = F)
  colnames(ld_file) <- c("CHR_A", "POS_A", "SNP_A",
                         "CHR_B", "POS_B", "SNP_B", "r2")
  
  # If metric for buddy is "r2" (correlation)
  if (metric == "r2") {
    ...
    # Return the SNP physically closest to lead SNP
    ld_file <- ld_file %>% 
    ...
    colnames(return_nearest) <- c("SNP_og", "CHR_og", "POS_og",
                                  "SNP_buddy", "CHR_buddy", "POS_buddy", 
                                  "dist_to_novel")
  } else { stop("Specify valid metric") }
...
getMAFGeneConsequenceForSNP <- function (to_annot) {
  res <- getBM(attributes = c("refsnp_id", "chr_name", "chrom_start", 
               ...
               filters = c("chr_name", "start", "snp_filter"),
               values = list(to_annot$chr, 
                             to_annot$pos, 
                             to_annot$snpid),
               mart = bm)
  return (res)
...
to_write <- annotated_full %>%
  select(any_of(c("SNP_og", "CHRPOS_og", "MAF_og", "GENE_og", "CONSEQUENCE_og",
                  "dist_to_novel", "r2",
