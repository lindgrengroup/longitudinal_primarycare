...
# Get coefficients for clustering
cdat <- lapply(SEX_STRATA, function (sx) {
  read.table(paste0("/well/lindgren/UKBIOBANK/samvida/adiposity/splines/new_models/",
                    PHENO, "/coefficients_", PHENO, "_", sx, ".txt"),
             header = T, sep = "\t", stringsAsFactors = F)
})
names(cdat) <- SEX_STRATA

# Get raw data for plotting
raw_dat <- readRDS(paste0("/well/lindgren/UKBIOBANK/samvida/adiposity/adj_traits/adj_",
                          PHENO, ".rds"))

# Create clustering log file
log_file <- paste0(PHENO, "/clustering_log_", PHENO, ".txt")
...
# Visualise BIC
pdf(paste0(PHENO, "/mclust_plots_", PHENO, ".pdf"), onefile = T)
...
clust_plots <- lapply(SEX_STRATA, function (sx) {
  ...
  for_plot <- raw_dat[[sx]][, c("eid", "age_event", "value")]
...
pdf(paste0(PHENO, "/clustplot_", PHENO, ".pdf"), onefile = T)
print(clust_plots)
dev.off()
