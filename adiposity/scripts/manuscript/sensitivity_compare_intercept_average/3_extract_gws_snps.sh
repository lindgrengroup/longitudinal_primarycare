...
for STRATA in BMI_F BMI_M BMI_sex_comb Weight_F Weight_M Weight_sex_comb; do
	...
	# Remove duplicates
	sort -u post_GWAS/gws_sig_snps/tmp_snp_list_${STRATA}.txt > post_GWAS/gws_sig_snps/snp_list_${STRATA}.txt
	...
	grep -F -w -f post_GWAS/gws_sig_snps/snp_list_${STRATA}.txt BOLT_results/${STRATA}_b0_final.txt \
	...
	grep -F -w -f post_GWAS/gws_sig_snps/snp_list_${STRATA}.txt BOLT_results/${STRATA}_average_trait_final.txt \
	>> post_GWAS/gws_sig_snps/${STRATA}_average_trait_gws_sig_hits.txt
done

rm post_GWAS/gws_sig_snps/tmp_snp_list_*
rm post_GWAS/gws_sig_snps/snp_list_*
