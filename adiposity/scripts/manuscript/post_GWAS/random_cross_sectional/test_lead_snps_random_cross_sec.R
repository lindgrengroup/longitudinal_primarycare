...
# Function to select random cross-sectional value for each individual
getCrossSecValue <- function (long_df) {
  cross_sec_df <- long_df %>% 
    group_by(eid) %>%
    slice_sample(n = 1)
  return (cross_sec_df)
}
...
full_res <- lapply(PHENOTYPES, function (p) {
  per_sex <- lapply(SEX_STRATA, function (sx) {
    ...
    per_varid <- lapply(VARIDS, function (v) {
      ...
      # Run this 10 times to get different sets of random values each time
      per_rep_res <- lapply(1:NREPS, function (i) {
        to_mod <- getCrossSecValue(full_df)
        test_res <- quantTest(to_mod, sx)
        test_res$iteration <- i
        return (test_res)
      })
      per_rep_res <- bind_rows(per_rep_res)
      per_rep_res$varid <- v
      return (per_rep_res)
    })
...
write.table(full_res,
            "/well/lindgren-ukbb/projects/ukbb-11867/samvida/adiposity/2211_models/GWAS/post_GWAS/cross_sec_in_same_data_replication.txt",
