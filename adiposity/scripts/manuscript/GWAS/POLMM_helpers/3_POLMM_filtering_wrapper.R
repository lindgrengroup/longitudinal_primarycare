...
# Create chr pos, A1, A2 columns
GWAS_res <- GWAS_res %>% as_tibble() %>%
  separate(Info, c("CHR", "POS", "Allele1", "Allele2"))

to_numeric <- c("CHR", "POS", "AltFreq", "beta", "seBeta", "Pvalue")

GWAS_res <- GWAS_res %>% 
  select(c("Marker", "CHR", "POS", "Allele1", "Allele2", "AltFreq", 
           "beta", "seBeta", "Pvalue")) %>%
  mutate(across(all_of(to_numeric), as.numeric)) %>%
  mutate(maf = ifelse(AltFreq < 0.5, AltFreq, 1-AltFreq))
...
cleaned <- cleaned %>% arrange(CHR, POS) %>%
  mutate(SNP = ifelse(grepl("^rs", Marker), Marker, 
                      paste0("chr", sub("_.*", "", Marker))))

to_print <- cleaned[, c("Marker", "CHR", "POS", 
                        "Allele2", "Allele1", "AltFreq",
                        "beta", "seBeta", "Pvalue")]
