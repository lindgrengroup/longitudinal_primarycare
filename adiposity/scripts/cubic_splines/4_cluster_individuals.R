...
raw_dat <- readRDS("/well/lindgren/UKBIOBANK/samvida/full_primary_care/data/indiv_qcd_data.rds")[[p]]
...
all_BIC <- lapply(PHENOTYPES, function (p) {
  per_sex <- lapply(SEX_STRATA, function (sx) {
    ...
    # Visualise BIC
    pdf(paste0("/well/lindgren/UKBIOBANK/samvida/adiposity/gp_only/plots/cubic_splines/clustering_BIC_",
               p, "_", sx, ".pdf"))
    plot(res)
    dev.off()
    return ()
...
mod_BIC <- lapply(PHENOTYPES, function (p) {
  per_sex <- lapply(SEX_STRATA, function (sx) {
    ...
    res <- Mclust(blups[[p]][[sx]], 
                  ...
                  modelNames = NCLUSTERS_TABLE$ctype[NCLUSTERS_TABLE$strata == strata_nm])
    ...
    write.table(to_save, 
                paste0("/well/lindgren/UKBIOBANK/samvida/adiposity/gp_only/results/cubic_spline_clusters_",
                       p, "_", sx, ".txt"),
